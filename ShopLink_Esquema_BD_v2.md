...
**TECNOINCUBADORA** | PostgreSQL (Neon) | v1.3
...
## 5. Gestão Financeira — Visão Geral
...
## 6. Landing Page — Compra e Reserva Remota
...
## 7. Sincronização Offline-First (POS)
...
2. Implementar `vw_stock_atual`, `vw_stock_disponivel`, `vw_saldo_contas` e `vw_dre_mensal` como materialized views se volume for alto.
3. Definir API REST/GraphQL por módulo: `/produtos`, `/vendas`, `/caixa`, `/stock`, `/publico`, `/pedidos`, `/financeiro`.
4. Configurar crons: expiração de reservas, parcelas vencidas, geração de despesas recorrentes.
5. Seed de dados de teste por `tipo_loja` para validar as duas configurações.
